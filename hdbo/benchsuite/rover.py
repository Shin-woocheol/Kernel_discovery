import torch
import numpy as np
from hdbo.benchsuite.rover_utils import RoverDomain, PointBSpline, ConstObstacleCost, NegGeom, AABoxes, UnionGeom, AdditiveCosts, \
    ConstCost

from hdbo.benchsuite.helper import ConstantOffsetFn, NormalizedInputFn
...
class RoverBenchmark(Benchmark):
    def __init__(
        ...
        super().__init__(
            dim=N_WAYPOINTS * 2,
            ub=torch.tensor(self.raw_x_range[1], dtype=settings.DTYPE, device=settings.DEVICE),
            lb=torch.tensor(self.raw_x_range[0], dtype=settings.DTYPE, device=settings.DEVICE),
        )
    ...
    def __call__(self, X: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(batch_size):
            # Benchsuite's rover helper returns single value
            val = f(X[i].detach().cpu().numpy())
            results.append(val)
